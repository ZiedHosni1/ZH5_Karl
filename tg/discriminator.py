...
class DiscriminatorModel(LightningModule):
    ...
    def wgan_loss(self, outputs, labels):
        ...
        w_loss = torch.abs(
            torch.sum(neg) / (neg.shape[0] + 1e-10)
            - torch.sum(pos) / (pos.shape[0] + 1e-10)
        )
        return w_loss
    ...
    def masked_mean(self, encoded, mask):
        ...
        """
        non_mask = (
            mask.transpose(0, 1).unsqueeze(-1) == False
        )  # [batch_size, maxlength, 1] if Pad: 0, else 1
        masked_encoded = encoded * non_mask  # [batch_size, maxlength, d_model]
        ave = masked_encoded.sum(dim=1) / (
            non_mask.sum(dim=1) + 1e-9
        )  # [batch_size, d_model], add +1e-9 to avoid division by 0
    ...
    def forward(self, features):  # [batch_size, maxlength]
        ...
        embedded = self.embedding(features) * math.sqrt(
            self.d_model
        ...
        # If true: apply WGAN
        if self.dis_wgan:
            weight_loss = torch.sum(self.classifier.weight**2) / 2.0
            bias_loss = torch.sum(self.classifier.bias**2) / 2.0
