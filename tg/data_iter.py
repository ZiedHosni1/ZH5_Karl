...
class GenDataLoader(LightningDataModule):
    ...
    def setup(self):
        ...
        self.data = pd.read_csv(
            self.positive_file, nrows=self.val_size + self.train_size, names=["smiles"]
        )
        ...
        val_idxs, train_idxs = (
            idxs[: self.val_size],
            idxs[self.val_size : self.val_size + self.train_size],
        )
...
class DisDataLoader(LightningDataModule):
    ...
    def custom_collate_and_pad(self, batch):
        ...
        # Pad the different lengths of tensors to the maximum length (each column is a sequence)
        tensors = torch.nn.utils.rnn.pad_sequence(tensors).transpose(
            0, 1
    ...
    def setup(self):
        ...
        self.labels = pd.DataFrame(
            [1 for _ in range(len(self.positive_data))]
            + [0 for _ in range(len(self.negative_data))],
            columns=["labels"],
        )
