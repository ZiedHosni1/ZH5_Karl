...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Directory containing the log files (gen_pretrain_logs, dis_pretrain_logs, adversarial_losses.csv)",
)
...
def plot_losses(df, train_loss_col, val_loss_col, title, filename):
    ...
    try:
        ...
        print(
            f"KeyError while plotting '{title}': {e}. Check if columns exist in the CSV."
        )
...
if gen_metrics_file and os.path.exists(gen_metrics_file):
    try:
        ...
        if (
            "gen_pre_train_loss" in gen_df.columns
            and "gen_pre_val_loss" in gen_df.columns
        ):
            plot_losses(
                gen_df,
                "gen_pre_train_loss",
                "gen_pre_val_loss",
                "Generator Pre-training Loss",
                "gen_pretrain_loss.pdf",
            )
        else:
        ...
        print(
            f"Error reading or processing Generator metrics file {gen_metrics_file}: {e}"
        )
else:
...
if dis_metrics_file and os.path.exists(dis_metrics_file):
    try:
        ...
        # Check if required columns exist
        if (
            "dis_pre_train_loss" in dis_df.columns
            and "dis_pre_val_loss" in dis_df.columns
        ):
            plot_losses(
                dis_df,
                "dis_pre_train_loss",
                "dis_pre_val_loss",
                "Discriminator Pre-training Loss",
                "dis_pretrain_loss.pdf",
            )
        else:
        ...
        print(
            f"Error reading or processing Discriminator metrics file {dis_metrics_file}: {e}"
        )
else:
            ...
            with plt.style.context(["science", "grid"]):
                ...
                if "epoch" in adv_df.columns:
                    ...
                    if not adv_plot_data.empty:
                        ax.plot(
                            adv_plot_data["epoch"],
                            adv_plot_data["gen_pg_loss"],
                            label="Generator PG Loss",
                            marker="o",
                            markersize=3,
                            linestyle="-",
                        )
                        ...
                        print(
                            "No valid data points for adversarial generator loss plot."
                        )
                else:
