...
# ============================================================================
# Define the model that deep copied from the generator for rollout sampling
...
class Rollout(object):
    def __init__(self, gen, roll_model, tokenizer, update_rate, device):
        self.ori_model = (
            gen  # Shallow copy: if model's parameters change, ori_model will change
        )
    ...
    def get_reward(
        self, samples, rollsampler, rollout_num, dis, dis_lambda=0.5, properties=None
    ):
        """
        ...
        """
        encoded = [
            torch.tensor(self.tokenizer.encode(s)) for s in samples
        ]  # Within the start and end token
        paded = (
            torch.nn.utils.rnn.pad_sequence(encoded).squeeze().to(self.device)
        )  # [max_len, batch_size]
        ...
        for i in range(rollout_num):
            ...
            for given_num in range(init, seq_len):
                ...
                generated_encoded = [
                    torch.tensor(self.tokenizer.encode(s))[1:-1]
                    for s in generated_smiles
                ]  # Remove the start token and end token and as the input of dis
                generated_paded = (
                    torch.nn.utils.rnn.pad_sequence(generated_encoded)
                    .squeeze()
                    .transpose(0, 1)
                    .to(self.device)
                )  # [batch_size, max_len]
                ...
                if dis_lambda != 1.0:
                    ...
                    if len(generated_smiles):  # batch_size
                        pct_unique = len(list(set(generated_smiles))) / float(
                            len(generated_smiles)
                        )
                        weights = np.array(
                            [
                                pct_unique / float(generated_smiles.count(sm))
            ...
            # For the last token
            last_encoded = [
                torch.tensor(self.tokenizer.encode(s))[1:-1] for s in samples
            ]
            last_paded = (
                torch.nn.utils.rnn.pad_sequence(last_encoded).squeeze().to(self.device)
            )
            pred = dis.forward(
                last_paded.transpose(0, 1)
            )  # [batch_size, max_len] -> [batch_size, 2]
            ...
            if dis_lambda != 1.0:
                ...
                weights = np.array(
                    [pct_unique / float(samples.count(s)) for s in samples]
                )
        ...
        if self.reward_ema is None:
            ...
            self.reward_ema = (
                self.ema_decay * self.reward_ema
                + (1 - self.ema_decay) * current_mean_reward
            )

        rewards = rewards - self.reward_ema  # Use EMA as baseline
