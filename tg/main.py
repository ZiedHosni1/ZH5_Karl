...
def main():
    ...
    with open(adv_log_file, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["epoch", "g_step", "gen_pg_loss"])  # Add g_step
    ...
    adv_dis_train_log_dir = os.path.join(PATHS, "adv_dis_train_logs")
    adv_dis_logger = CSVLogger(save_dir=adv_dis_train_log_dir, name="adv_dis_metrics")
    adv_trainer = pytorch_lightning.Trainer(
        max_epochs=D_STEP,
        gpus=GPUS,
        logger=adv_dis_logger,
        weights_summary=None,
        ...
        for epoch in range(args.adv_epochs):
            ...
            for g_step in range(G_STEP):
                ...
                with open(adv_log_file, "a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([epoch + 1, g_step + 1, loss.item()])
