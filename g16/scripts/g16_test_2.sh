...
echo "Starting post-processing: cubegen, VMD plots..."

# Check if fchk exists and is non-empty before proceeding
# This check assumes g16 ran successfully and created the fchk with the -fchk flag
if [ -s "${INPUT_NAME}.fchk" ]; then # Changed to -s to check for non-zero size
...
mol addfile ${INPUT_NAME}.fchk type gaussianfchk waitfor all

# Set representation for the orbital/property (Isosurface)
mol delrep 0 top
mol representation Isosurface 0.03 0 0 1 1 1 # Adjust isovalue (0.03) if needed
mol color Name # Color the isosurface (e.g., by potential)
mol addrep top

# Add representation for the molecule itself (e.g., Licorice)
# Adjust parameters (bond radius 0.1, atom/bond resolution 12) as needed
mol representation Licorice 0.100000 12.000000 12.000000
mol color Element # Color atoms by element type
mol addrep top
...
EOF
            ...
            if [ -f "${prop}.eps" ]; then
                ...
                # Ghostscript should already be loaded
                gs -dBATCH -dNOPAUSE -sDEVICE=pdfwrite -sOutputFile=${prop}.pdf ${prop}.eps
                if [ $? -ne 0 ]; then echo "Warning: Ghostscript failed to create ${prop}.pdf"; fi
                gs -dBATCH -dNOPAUSE -sDEVICE=svg -sOutputFile=${prop}.svg ${prop}.eps
                if [ $? -ne 0 ]; then echo "Warning: Ghostscript failed to create ${prop}.svg"; fi
            else
    ...
    # Check if the necessary fchk file still exists (redundant check, but safe)
    # Note: This inner check reuses the condition from the outer 'if', so it's guaranteed true here
    # Create VMD Tcl script for the molecule
    cat >vmd_molecule.tcl <<EOF
# Load the geometry from the fchk file
mol new ${INPUT_NAME}.fchk type gaussianfchk waitfor all
...
EOF
    ...
    if [ -f "molecule.eps" ]; then
        ...
        # Ghostscript should already be loaded
        gs -dBATCH -dNOPAUSE -sDEVICE=pdfwrite -sOutputFile=molecule.pdf molecule.eps
        if [ $? -ne 0 ]; then echo "Warning: Ghostscript failed to create molecule.pdf"; fi
        gs -dBATCH -dNOPAUSE -sDEVICE=svg -sOutputFile=molecule.svg molecule.eps
        if [ $? -ne 0 ]; then echo "Warning: Ghostscript failed to create molecule.svg"; fi
    else
...
# This 'else' matches the main 'if [ -s "${INPUT_NAME}.fchk" ]' check
    ...
    echo "Skipping post-processing (cubegen, VMD) because ${INPUT_NAME}.fchk was not found or is empty."
fi
